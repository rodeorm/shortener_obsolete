...
func (s postgresStorage) InsertUser(Key int) (*User, error) {
	...
	if Key == 0 {
		...
		RETURNING id`
		id := 0
		err := s.DB.QueryRowContext(ctx, sqlStatement, time.Now().Format(time.DateTime)).Scan(&id)
		if err != nil {
...
func (s postgresStorage) SelectUserURLHistory(Key int) (*[]UserURLPair, error) {
	...
	rows, err := s.DB.QueryContext(context.TODO(), "SELECT original, short, userID FROM Urls WHERE UserID = $1", fmt.Sprint(Key))
	if err != nil {
		return nil, err
	}
...
func (s postgresStorage) deleteURL(url, userKey string) (bool, error) {
	...
	_, err = stmt.ExecContext(context.TODO(), url, userKey)
	if err != nil {
		return false, err
	}
	err = tx.Commit()
